...
if ! gcloud compute target-http-proxies describe $PROXY_NAME --region=$REGION --project=$PROJECT_ID &> /dev/null; then
    echo "Creating HTTP Proxy '$PROXY_NAME'..."
    gcloud compute target-http-proxies create $PROXY_NAME \
        --url-map $URL_MAP_NAME \
        --region=$REGION \
        --project=$PROJECT_ID || { echo "Failed to create HTTP Proxy."; exit 1; }
else
    echo "HTTP Proxy '$PROXY_NAME' already exists."
    CURRENT_URL_MAP=$(gcloud compute target-http-proxies describe $PROXY_NAME --region=$REGION --project=$PROJECT_ID --format="value(urlMap)" | awk -F'/' '{print $NF}')
...
echo "Internal HTTP Proxy configured."
...
gcloud run services update $FE_SVELTE_SERVICE_NAME \
    --update-env-vars PUBLIC_API_URL="http://$INTERNAL_HOSTNAME" \
    --platform managed \
    --region $REGION \
    --ingress "all" \
...
echo "Your internal API endpoint is: http://$INTERNAL_HOSTNAME"
echo "Cloud Run URLs:"
